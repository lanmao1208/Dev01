...
# 根据使用场景的不同选择不同的APIView版本继承、
# 1.RetrieveModelMixin
#   提供retrieve(request, *args, **kwargs)方法
#   获取已存在的数据详情(一条数据)
#   获取成功200K,不成功404
# 2.UpdateModelMixin
#   提供updata(request, *args, **kwargs)方法用于全更新
#   提供partial_update(request, *args, **kwargs)用于部分更新，支持PATCH方法
#   更新已存在的模型实例(一条数据)
#   成功200K, 不成功404，参数有误400
# 3.DestroyModelMixin
...
#   创建新的模型实例(创建新的记录)
#   创建成功201，请求参数有误400
class InterfacesViewLast(generics.ListCreateAPIView):
    """
    """
    queryset = Interfaces.objects.all()
    serializer_class = InterfacesModelSerializer
...
# GenericAPIView和APIView只支持，对get、post、put、delete、patch等请求方法
# 如果要支持action，那么需要继承ViewSet
# 当前ViewSet，无法支持.get_object()、.filter_queryset()、.paginate_queryset()
# class InterfacesViewSet(viewsets.ViewSet):
# GenericViewSet才支持对列表数据进行过滤、排序、分页操作
# class InterfacesViewSet(mixins.ListModelMixin,
#                       mixins.RetrieveModelMixin,
#                       mixins.CreateModelMixin,
#                       mixins.UpdateModelMixin,
#                       mixins.DestroyModelMixin,
#                       viewsets.GenericViewSet):
# 只获取数据而不是修改的话，可以只继承viewsets.ReadOnlyModelViewSet
# 获取单个或者多个数据，该父类只有retrieve和list方法
